...
export const NumberInput = ({ label, icon, id, ...props }) => {
    const inputId = id || props.name || `input-${label?.replace(/\s+/g, '-').toLowerCase()}`;
    return (_jsxs("div", { children: [_jsx("label", { htmlFor: inputId, className: "block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2", children: label }), _jsxs("div", { className: "relative", children: [icon && (_jsx("div", { className: "absolute inset-y-0 left-0 pl-3 flex items-center", children: icon })), _jsx("input", { id: inputId, type: "number", ...props, className: `w-full ${icon ? 'pl-10' : 'pl-4'} py-3 
